...
function toast(msg){
  ...
  /* ===== Malayalam → Manglish (Kerala realistic WhatsApp style) ===== */
  function malToManglish(txt){
    ...
    const VOW_SIGNS = {
      ...
      "െ":"e","േ":"e","ൈ":"ai","ൊ":"o","ോ":"o","ൗ":"au","ൃ":"ri"
    };
    const CONS = {
      "ക":"k","ഖ":"kh","ഗ":"g","ഘ":"gh","ങ":"ng",
      "ച":"ch","ഛ":"chh","ജ":"j","ഝ":"jh","ഞ":"nj",
      "ട":"d","ഠ":"dh","ഡ":"d","ഢ":"dh","ണ":"n",
      "ത":"th","ഥ":"thh","ദ":"d","ധ":"dh","ന":"n",
      "പ":"p","ഫ":"ph","ബ":"b","ഭ":"bh","മ":"m",
      "യ":"y","ര":"r","ല":"l","വ":"v",
      "ശ":"sh","ഷ":"sh","സ":"s","ഹ":"h",
      "ഴ":"zh","ള":"l","റ":"r","ൺ":"n","ൻ":"n","ർ":"r","ൽ":"l","ൾ":"l"
    };
    const VIR = "്";
    let out = "";
    ...
    // cleanup: fix common patterns (like 'aa' merge, 'thh'→'th')
    out = out
      .replace(/thh/g,"th")
      .replace(/([aeiou])a(\b|[^a-z])/gi,"
$2"); // avoid trailing 'a' doubling
    return out;
  }
  ...
  // === Global UI Rebinding after DOM ready ===
  document.addEventListener("DOMContentLoaded", ()=>{
    // Re-grab elements (handles sidebar or phone)
    ...
    // Guard: only if script-defined functions exist
    if(typeof playFromCurrent === "function"){
      if(btnPlay)  btnPlay.onclick  = ()=>{ paused=false; playFromCurrent(); };
      if(btnPause) btnPause.onclick = ()=> pausePlayback();
      if(btnReset) btnReset.onclick = ()=> resetPlayback();
    }

    if(typeof loadScriptFromTextarea === "function"){
      if(btnLoad)   btnLoad.onclick   = ()=> loadScriptFromTextarea();
      if(btnExport) btnExport.onclick = ()=> exportCurrent();
    }

    // Visual confirm in console
    console.log("✅ UI buttons rebound after DOM ready.");
  });
/* ==== DIAGNOSTIC & REBIND PATCH (markerless) ==== */
(function(){
  function ts(){try{const d=new Date();return d.toISOString().split("T")[1].slice(0,12)}catch(e){return""}}
  function log(){try{console.log("[SIM "+ts()+"]",...arguments)}catch(e){}}
  window.onerror=(m,s,l,c,e)=>console.error("[SIM ERR]",m,"@"+l+":"+c,e&&e.stack);

  function expose(){
    const map={play:"playFromCurrent",pause:"pausePlayback",reset:"resetPlayback",load:"loadScriptFromTextarea",exp:"exportCurrent"};
    for(const k in map){
      const fn=map[k];
      if(typeof window[fn]!=="function"&&typeof globalThis[fn]==="function"){
        window[fn]=globalThis[fn];
      }
    }
    log("exposed fns:",Object.keys(map).map(k=>[k,typeof window[map[k]]]));
  }

  function bind(){
    log("bind start");
    const ids=["btnPlay","btnPause","btnReset","btnLoad","btnExport"];
    const found={}; ids.forEach(id=>found[id]=!!document.getElementById(id));
    log("found buttons",found);
    expose();
    const B=(id,fn)=>{const e=document.getElementById(id);if(e&&typeof fn==="function"){e.onclick=()=>{log("CLICK",id);try{fn()}catch(x){console.error("[SIM]",id,"err",x)}}}};
    if(window.playFromCurrent){B("btnPlay",()=>{window.paused=false;window.playFromCurrent()})}
    if(window.pausePlayback){B("btnPause",()=>window.pausePlayback())}
    if(window.resetPlayback){B("btnReset",()=>window.resetPlayback())}
    if(window.loadScriptFromTextarea){B("btnLoad",()=>window.loadScriptFromTextarea())}
    if(window.exportCurrent){B("btnExport",()=>window.exportCurrent())}
    log("bind done");
  }

  // retry a few times just in case
  document.addEventListener("DOMContentLoaded",()=>{log("DOMContentLoaded");bind();setTimeout(()=>bind(),1000);setTimeout(()=>bind(),3000)});
  window.addEventListener("load",()=>{log("window load");bind()});
})();
